...
<!-- Primary Meta Tags -->
    <title>Tứ Diện Đều - Công Thức, Tính Toán & Lời Giải Chi Tiết</title>
    <meta name="title" content="Tứ Diện Đều - Công Thức, Tính Toán & Lời Giải Chi Tiết">
    <meta name="description" content="Công thức tính nhanh thể tích, chiều cao, diện tích toàn phần của khối tứ diện đều cạnh a ôn thi THPT.">
...
<body>
    ...
    <div class="container">
        ...
        <% } %>
        <form class="calc-form" method="post" action="${pageContext.request.contextPath}/tu-dien-deu">
            <div class="form-row">
                <label>Cạnh (a):</label>
        ...
        <div class="result-box">
            ...
            <div class="step-detail">
                ...
                <p>\(V = \dfrac{a^3\sqrt{2}}{12} = \dfrac{${param.a}^3\sqrt{2}}{12} \approx \) <strong><fmt:formatNumber value="${requestScope.tdd.getVolume()}" pattern="#,##0.####"/></strong> <span class="unit-text">đơn vị thể tích</span></p>
            </div>
            <div class="step-detail">
                <p><strong>2. Diện tích toàn phần (S_tp):</strong></p>
                <p>\(S_{tp} = a^2\sqrt{3} = ${param.a}^2\sqrt{3} \approx \) <strong><fmt:formatNumber value="${requestScope.tdd.getSurfaceArea()}" pattern="#,##0.####"/></strong> <span class="unit-text">đơn vị diện tích</span></p>
            </div>
            <div class="step-detail">
                <p><strong>3. Diện tích 1 mặt:</strong></p>
                <p>\(S_1 = \dfrac{a^2\sqrt{3}}{4} = \dfrac{${param.a}^2\sqrt{3}}{4} \approx \) <strong><fmt:formatNumber value="${requestScope.tdd.getFaceArea()}" pattern="#,##0.####"/></strong> <span class="unit-text">đơn vị diện tích</span></p>
            </div>
            <div class="step-detail">
                <p><strong>4. Chiều cao (h):</strong></p>
                <p>\(h = \dfrac{a\sqrt{6}}{3} = \dfrac{${param.a}\sqrt{6}}{3} \approx \) <strong><fmt:formatNumber value="${requestScope.tdd.getHeight()}" pattern="#,##0.####"/></strong> <span class="unit-text">đơn vị dài</span></p>
            </div>
            <div class="step-detail">
                <p><strong>5. Bán kính cầu ngoại tiếp (R):</strong></p>
                <p>\(R = \dfrac{a\sqrt{6}}{4} = \dfrac{${param.a}\sqrt{6}}{4} \approx \) <strong><fmt:formatNumber value="${requestScope.tdd.getCircumradius()}" pattern="#,##0.####"/></strong> <span class="unit-text">đơn vị dài</span></p>
            </div>
            <div class="step-detail">
                <p><strong>6. Bán kính cầu nội tiếp (r):</strong></p>
                <p>\(r = \dfrac{a\sqrt{6}}{12} = \dfrac{${param.a}\sqrt{6}}{12} \approx \) <strong><fmt:formatNumber value="${requestScope.tdd.getInradius()}" pattern="#,##0.####"/></strong> <span class="unit-text">đơn vị dài</span></p>
            </div>
        </div>
        <% } %>
